...
export default {
    ...
    drawSiteStaion(myMap){
      ...
      L.featureGroup(test_click).on('mouseover', function(place){
        ...
        poi_click.on('popupopen', function(e) {
          ...
          let mychart1
          eval("let myChart" +  a_num.toFixed(0) + "= echarts.init(document.getElementById('markerguomao"+ placeName+"\'));mychart1=myChart"+a_num.toFixed(0));
          // 指定图表的配置项和数据
          let option = {
            textStyle:{
  ...
  mounted() {
    ...
    this.drawSiteStaion(myMap)//绘制地铁站
