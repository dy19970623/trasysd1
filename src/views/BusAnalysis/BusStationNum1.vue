...
export default {
  ...
  methods: {
    getmap() {
      ...
      // 指定图表的配置项和数据
      BusStationNum1.setOption(this.BusStationNum_option1)
      var map=BusStationNum1.getModel().getComponent('mapbox3D').getMapbox();
      //实例化导航控件
        ...
        document.getElementById("charts").style.visibility="visible";
	      ...
	      for(var j=0;j<BSNum.finalnums.length;j++){
	        if(BSNum.finalnums[j].siteName==keyWord2){
	            ...
	            BusStationNum1.setOption(_this.BusStationNum_option1);
	            break;
	        }
	      }
